...
function storePartialArchiveState(hit, partial) {
  ...
  if (partial && typeof partial === 'object') {
    ...
    void savePartialArchives().catch(() => {});
    return sanitized;
  }
...
const DEFAULT_SETTINGS = {
  ...
  uploadMetaJson: false,
  uploadIndexM3u8: false,
  uploadMasterSourceM3u8: false,
  uploadIssuesTxt: true,
  perItemDelayMs: 0,
};
...
const STATE_KEYS = {
  ...
  hits: 'sfa_hits',
  config: 'sfa_config',
  partialArchives: 'sfa_partial_archives'
};
...
function headerSafeValue(value, fallback = 'upload.bin') {
...
function binarySourceSize(value) {
  const source = normalizeBinarySource(value);
  if (!source) return 0;
  if (typeof Blob !== 'undefined' && source instanceof Blob) return Number(source.size || 0) || 0;
  if (source instanceof ArrayBuffer) return Number(source.byteLength || 0) || 0;
  if (ArrayBuffer.isView(source)) return Number(source.byteLength || 0) || 0;
  if (typeof source.size === 'number') return Number(source.size || 0) || 0;
  if (typeof source.byteLength === 'number') return Number(source.byteLength || 0) || 0;
  return 0;
}
...
async function fetchWithRetry(url, opts = {}, kind = 'text', label = 'resource') {
  ...
  for (let attempt = 0; attempt < ARCHIVE_TUNING.retryDelaysMs.length; attempt++) {
    ...
    try {
      const res = await openResourceResponse(url, opts);
      if (!res.ok) throw new Error(`HTTP ${res.status} for ${url}`);
      const contentType = res.headers.get('content-type') || '';
      const finalUrl = res.headers.get('x-libraryjs-final-url') || res.url || url;
      if (kind === 'blob') {
...
function normalizeFFmpegBaseUrl(raw) {
  const txt = safeText(raw, '').trim();
  if (!txt) return '';
  try {
...
function requestHeaderLookupKeys(url = '', pageUrl = '') {
  const keys = [];
  const seen = new Set();
  const add = (value) => {
  ...
  const addOrigin = (value) => {
    ...
    if (!text) return;
    try {
      add(`__origin__:${new URL(text).origin}`);
    } catch {}
  };
  add(url);
  add(pageUrl);
...
function mergedRequestHeadersFor(url, { pageUrl = '' } = {}) {
  const store = MEMORY.requestHeaders instanceof Map ? MEMORY.requestHeaders : null;
  if (!store) return [];
  const sources = [];
  for (const key of requestHeaderLookupKeys(url, pageUrl)) {
    const raw = store.get(key);
    if (Array.isArray(raw) && raw.length) sources.push(raw);
  }
  return sources;
}

function requestHeadersFor(url, { allowRange = false, pageUrl = '' } = {}) {
  const out = {};
  for (const raw of mergedRequestHeadersFor(url, { pageUrl })) {
    for (const h of raw) {
...
function requestHeadersForProxy(url, { allowRange = false, pageUrl = '' } = {}) {
  const out = {};
  for (const raw of mergedRequestHeadersFor(url, { pageUrl })) {
    for (const h of raw) {
      const name = String(h?.name || '').trim();
      ...
      if (!allowRange && lower === 'range') continue;
      if (!(name in out)) out[name] = String(h?.value ?? '');
    }
  }
  const ref = String(pageUrl || '').trim();
  if (ref) {
    try {
      const origin = new URL(ref).origin;
      if (!out.referer) out.referer = ref;
...
async function openResourceResponse(url, opts = {}) {
  const targetUrl = String(url || '').trim();
  const pageUrl = String(opts.referrer || opts.pageUrl || '').trim();
  const useProxy = !!MEMORY.config.serverOrigin
    && !!targetUrl
    && isHttpUrl(targetUrl)
    && !isSameOriginUrl(targetUrl, MEMORY.config.serverOrigin)
    && opts.useServerDownload !== false;

  const directInit = { cache: 'no-store', credentials: 'include' };
  const signal = opts.signal || opts.archiveSignal || null;
  if (signal) directInit.signal = signal;
  if (pageUrl) directInit.referrer = pageUrl;
  if (opts.headers && Object.keys(opts.headers).length) directInit.headers = opts.headers;

  if (useProxy) {
    const proxyHeaders = requestHeadersForProxy(targetUrl, {
    ...
    if (opts.bodyText != null) payload.bodyText = opts.bodyText;
    if (opts.bodyBase64 != null) payload.bodyBase64 = opts.bodyBase64;
    try {
      const proxyResponse = await fetch(joinUrl(MEMORY.config.serverOrigin, 'download'), {
        method: 'POST',
        cache: 'no-store',
        credentials: 'omit',
      ...
      throw new Error(`HTTP ${proxyResponse.status} via server download for ${targetUrl}`);
    } catch (err) {
      // Fall through to direct fetch.
    }
  }

  return await fetch(targetUrl, directInit);
}
...
function buildArchiveBase(hit) {
  ...
  const label = sanitizeSegment(`${nowIsoStamp()}_${unique}_${host}_${filenameFromUrl(labelSource, 'stream')}`);
  return ensureSlash(joinPath(normalizeArchiveFolder(MEMORY.settings.archiveFolder || '/videodownloader/'), label), true, true);
}

function buildArchiveJobFolder(hit, base = '/libraryjs-upload-temp/') {
  const host = (() => { try { return new URL(hit.pageUrl || hit.sourceUrl || hit.url).hostname; } catch { return 'site'; } })();
  const labelSource = hit?.sourceUrl || hit?.url || 'stream';
  const unique = `${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`;
  const label = sanitizeSegment(`${nowIsoStamp()}_${unique}_${host}_${filenameFromUrl(labelSource, 'stream')}`);
...
async function loadState() {
  const [{ [STATE_KEYS.settings]: settings, [STATE_KEYS.config]: config, [STATE_KEYS.partialArchives]: partialArchives }, { [STATE_KEYS.hits]: sessionHits }] = await Promise.all([
    storageGet([STATE_KEYS.settings, STATE_KEYS.config, STATE_KEYS.partialArchives], 'local'),
  ...
  MEMORY.partialArchives = new Map(
    Object.entries(partialArchives || {}).filter(([key, value]) => key && value && typeof value === 'object')
  );
  if (!(MEMORY.partialArchiveBlobs instanceof Map)) MEMORY.partialArchiveBlobs = new Map();
...
async function saveSettings(patch) {
  ...
  MEMORY.settings.uploadMetaJson = !!MEMORY.settings.uploadMetaJson;
  MEMORY.settings.uploadIndexM3u8 = !!MEMORY.settings.uploadIndexM3u8;
  MEMORY.settings.uploadMasterSourceM3u8 = !!MEMORY.settings.uploadMasterSourceM3u8;
  MEMORY.settings.uploadIssuesTxt = MEMORY.settings.uploadIssuesTxt !== false;
...
function cloneHitForSession(hit) {
  if (!hit || typeof hit !== 'object') return hit;
  const cloned = { ...hit };
  delete cloned.partialArchive;
  if (typeof cloned.text === 'string' && cloned.text.length > 65536) {
    cloned.text = `${cloned.text.slice(0, 65536)}…`;
  }
  if (typeof cloned.error === 'string' && cloned.error.length > 16384) {
    cloned.error = `${cloned.error.slice(0, 16384)}…`;
  }
  if (Array.isArray(cloned.variants) && cloned.variants.length > 50) {
    cloned.variants = cloned.variants.slice(0, 50);
  }
  return cloned;
}

function serializePartialArchiveStore() {
  const out = {};
  for (const [key, partial] of MEMORY.partialArchives.entries()) {
    if (!key) continue;
    out[key] = clonePartialArchiveWithoutBlobs(partial);
  }
  return out;
}

async function savePartialArchives() {
  await chrome.storage.local.set({ [STATE_KEYS.partialArchives]: serializePartialArchiveStore() });
}

async function saveHits() {
  const payload = Array.isArray(MEMORY.hits) ? MEMORY.hits.map(cloneHitForSession) : [];
  try {
    await chrome.storage.session.set({ [STATE_KEYS.hits]: payload });
    return true;
  } catch (err) {
    const msg = String(err?.message || err || '');
    if (!/quota|exceeded|maximum|too large|bytes/i.test(msg)) throw err;
    const compactPayload = payload.map((hit) => {
      const cloned = { ...hit };
      delete cloned.text;
      delete cloned.error;
    ...
    try {
      await chrome.storage.session.set({ [STATE_KEYS.hits]: compactPayload });
      return true;
    } catch (fallbackErr) {
      console.warn('saveHits fallback failed:', fallbackErr);
      return false;
    }
  }
}
...
function normalizeUploadQueueKey(targetDir) {
  const raw = String(targetDir || '').trim() || '/videodownloader/';
  if (/^https?:\/\//i.test(raw)) {
    try {
      const url = new URL(raw);
      return `${url.origin}${ensureSlash(url.pathname, true, true)}`;
    } catch {
      return raw;
    }
  }
  return normalizeArchiveFolder(raw);
}

function getUploadQueueLimit() {
  const configured = Number(MEMORY.settings.uploadConcurrency || MEMORY.settings.uploadParallelism || 0) || 0;
  if (configured > 0) return Math.max(1, Math.min(10, Math.floor(configured)));
  return String(MEMORY.config.platform || 'windows').toLowerCase() === 'android' ? 10 : 2;
}

async function withUploadQueue(queueKey, task) {
  const key = String(queueKey || '').trim() || '__default__';
  const limit = getUploadQueueLimit();
  const state = UPLOAD_QUEUE_STATE.get(key) || { active: 0, waiters: [] };
  UPLOAD_QUEUE_STATE.set(key, state);
...
async function uploadSliceWithFetch(fullPath, chunk, headers, options = {}) {
...
function shouldFallbackToXhr(err) {
  const message = String(err?.message || err || '');
  return /failed to fetch|networkerror|network error|load failed|TypeError: Failed to fetch/i.test(message);
}

async function uploadViaPutSlices(fullPath, blob, filename = '', options = {}) {
  const payload = normalizeUploadPayload(blob);
  const totalSize = Number(payload.size || 0) || 0;
  if (!totalSize) return true;

  const contentType = String(payload.type || 'application/octet-stream');
  const uploadId = createUploadSessionId();
  const chunkSize = Math.max(256 * 1024, Math.min(4 * 1024 * 1024, Number(MEMORY.settings.uploadChunkBytes || UPLOAD_SLICE_BYTES) || UPLOAD_SLICE_BYTES));
  const chunkCount = Math.max(1, Math.ceil(totalSize / chunkSize));

  let offset = 0;
  let index = 0;
  while (offset < totalSize) {
    const end = Math.min(totalSize, offset + chunkSize);
    const chunk = payload.slice(offset, end);
    const headers = {
      'Content-Type': contentType,
      'Content-Range': `bytes ${offset}-${end - 1}/${totalSize}`,
      'X-LibraryJS-Upload-Offset': String(offset),
      'X-LibraryJS-Upload-Id': uploadId,
      'X-LibraryJS-Upload-Name': headerSafeValue(filename || payload.name || 'upload.bin'),
      'X-LibraryJS-Chunk-Index': String(index),
      'X-LibraryJS-Chunk-Count': String(chunkCount),
      'X-LibraryJS-Upload-Size': String(totalSize)
    };

    const attempts = [
      () => uploadSliceWithFetch(fullPath, chunk, headers, options),
      () => uploadSliceWithFetch(fullPath, chunk, headers, options),
      () => uploadSliceWithFetch(fullPath, chunk, headers, options)
    ];
    ...
    for (let attempt = 0; attempt < attempts.length; attempt++) {
      try {
        res = await attempts[attempt]();
        if (res.ok) break;
        lastErr = new Error(`HTTP ${res.status}${res.text ? ` - ${res.text}` : ''}`);
      } catch (err) {
        lastErr = err;
      }
    ...
    if (!res || !res.ok) {
      throw new Error(`slice ${index + 1}/${chunkCount} failed: ${String(lastErr?.message || lastErr || 'network error')}`);
    }
...
async function uploadBlob(targetDir, filename, blob, options = {}) {
  const queueKey = normalizeUploadQueueKey(targetDir);
  return await withUploadQueue(queueKey, async () => {
...
function ensureMp4FallbackName(filename, fallbackBase = 'compiled') {
  const raw = String(filename || '').trim();
  if (!raw) return `${String(fallbackBase || 'compiled').replace(/\.[^./\\]+$/, '')}.mp4`;
  const withoutExt = raw.replace(/\.[^./\\]+$/, '');
  return `${withoutExt || String(fallbackBase || 'compiled').replace(/\.[^./\\]+$/, '')}.mp4`;
}

function createTransportStreamFallbackBlob(files) {
  const parts = [];
  for (const file of Array.isArray(files) ? files : []) {
    ...
    if (blob instanceof Blob) {
      ...
      parts.push(blob.buffer.slice(blob.byteOffset, blob.byteOffset + blob.byteLength));
    }
  }
  return new Blob(parts, { type: 'video/mp2t' });
}

function shouldUploadSidecar(filename) {
  const name = String(filename || '').trim().toLowerCase();
  switch (name) {
    case 'meta.json':
      return !!MEMORY.settings.uploadMetaJson;
    case 'index.m3u8':
      return !!MEMORY.settings.uploadIndexM3u8;
    case 'master-source.m3u8':
      return !!MEMORY.settings.uploadMasterSourceM3u8;
    case 'issues.txt':
      return MEMORY.settings.uploadIssuesTxt !== false;
    default:
      return true;
  }
}

function pushUploadIfEnabled(uploads, filename, blob) {
  if (blob && shouldUploadSidecar(filename)) uploads.push({ name: filename, blob });
}
...
async function fetchServerFileBlob(folder, filename, options = {}) {
  const url = serverFileUrl(folder, filename);
  if (!url) throw new Error('Server file URL unavailable.');
  const res = await fetch(url, { cache: 'no-store', signal: options.signal || undefined });
  if (!res.ok) throw new Error(`HTTP ${res.status} for ${filename}`);
  return await res.blob();
}

async function verifyUploadedFile(folder, filename, expectedSize = null, options = {}) {
  const url = serverFileUrl(folder, filename);
  if (!url) throw new Error('Server file URL unavailable.');

  let lastError = null;
  for (let attempt = 0; attempt < UPLOAD_VERIFY_RETRY_DELAYS_MS.length; attempt++) {
    ...
    try {
      ...
      if (res.ok) {
        if (Number.isFinite(Number(expectedSize)) && Number(expectedSize) > 0) {
          const rawSize = Number(res.headers.get('content-length') || 0) || 0;
          if (rawSize > 0 && rawSize !== Number(expectedSize)) {
            throw new Error(`Upload verification size mismatch for ${filename}: expected ${Number(expectedSize)} bytes, got ${rawSize}`);
          }
        }
        return true;
      }
      ...
      lastError = new Error(`Upload verification failed: HTTP ${status} for ${filename}`);
      if (![404, 409, 423, 502, 503, 504].includes(status)) {
        throw lastError;
      }
    } catch (err) {
      lastError = err;
    ...
    if (attempt < UPLOAD_VERIFY_RETRY_DELAYS_MS.length - 1 && delayMs > 0) {
      await sleep(delayMs);
    }
  }
...
async function waitForServerFile(folder, filename, options = {}) {
  const retries = Math.max(1, Number(options.retries || 4) || 4);
  const baseDelayMs = Math.max(0, Number(options.delayMs || 250) || 0);
  ...
  for (let attempt = 0; attempt < retries; attempt++) {
    try {
      ...
      const status = Number(res.status || 0) || 0;
      lastError = new Error(`Server file not visible yet: ${filename} (HTTP ${status})`);
      if (![404, 409, 423, 502, 503, 504].includes(status)) {
        throw lastError;
      }
    } catch (err) {
      lastError = err;
      const message = String(err?.message || err || '');
      if (!/HTTP\s+(?:404|409|423|502|503|504)/i.test(message) && !/network|fetch|failed/i.test(message)) {
        throw err;
      }
    }
...
async function fetchBlobMetaWithProgress(url, opts = {}, onProgress = null) {
  ...
  for (let attempt = 0; attempt < ARCHIVE_TUNING.retryDelaysMs.length; attempt++) {
    ...
    try {
      const response = await openResourceResponse(url, {
        ...opts,
        signal: opts.signal || opts.archiveSignal || (controller ? controller.signal : null)
      });
      if (!response.ok) throw new Error(`HTTP ${response.status} for ${url}`);
      const contentType = response.headers.get('content-type') || '';
      const contentLength = Number(response.headers.get('content-length') || 0) || 0;
      const finalUrl = response.headers.get('x-libraryjs-final-url') || response.url || url;
  ...
  for (let i = 0; i < selected.length; i++) {
    ...
    try {
      const fetched = await fetchBlobMeta(subHit.url, {
        ...
        headers: requestHeadersFor(subHit.url, { pageUrl: subHit.pageUrl || subHit.sourceUrl || primaryHit.pageUrl || primaryHit.sourceUrl || subHit.url })
      });
...
async function previewSubtitleHit(hit, options = {}) {
  ...
  const fetched = await fetchBlobMeta(hit.url, {
    ...
    headers: requestHeadersFor(hit.url, { pageUrl: hit.pageUrl || hit.sourceUrl || hit.url })
  });
  if (!fetched?.blob || fetched.blob.size <= 0) throw new Error('empty subtitle response');
...
async function archiveSubtitleHit(hit, options = {}) {
  ...
  pushUploadIfEnabled(uploads, 'meta.json', new Blob([JSON.stringify({
  ...
  if (warnings.length) pushUploadIfEnabled(uploads, 'issues.txt', new Blob([warnings.join('\n') + '\n'], { type: 'text/plain' }));
  ...
  await saveHits();
  const uploadConcurrency = Math.max(1, Math.min(getUploadQueueLimit(), uploads.length));
await runWithConcurrency(uploads, uploadConcurrency, async (item, i) => {
  ...
  try {
    const delayMs = getPerItemDelayMs();
    if (delayMs) await sleep(delayMs);
    const ok = await uploadBlob(item.targetDir || folder, item.name, item.blob, { signal: archiveSignal || undefined });
...
async function generatePlaceholderSegmentInBrowser({ duration, family, outputName, quality = '', resolution = '' }) {
  const outName = String(outputName || (String(family || '').toLowerCase() === 'ts' ? 'gap.ts' : 'gap.mp4'));
  const mimeType = String(outName.toLowerCase().endsWith('.ts') ? 'video/mp2t' : 'video/mp4');
  const bytes = new Uint8Array(Math.max(1, Math.min(1024, Math.round(Number(duration || 1) * 16))));
  return { name: outName, mimeType, blob: new Blob([bytes], { type: mimeType }) };
}
...
async function compileArchiveFromPartial(hit, options = {}) {
  ...
  const stagingFolder = options.stagingFolder || partial.stagingFolder || buildArchiveJobFolder(hit);
  ...
  if (rewrittenPlaylist) {
    sourceUploads.push({
      name: playlistName,
      blob: new Blob([rewrittenPlaylist], { type: 'application/vnd.apple.mpegurl' })
    });
  }
  ...
  for (let i = 0; i < fileEntries.length; i++) {
    ...
    sourceUploads.push({ name, blob });
  }

  const sourceUploadFailures = [];
  if (sourceUploads.length) {
    setArchiveStage(hit, 'archiving', 'Uploading source files');
    await updateHitProgress(hit, 0, fileEntries.length + 1, 'Uploading source files');
  }

  const sourceUploadConcurrency = Math.max(1, Math.min(getUploadQueueLimit(), sourceUploads.length));
await runWithConcurrency(sourceUploads, sourceUploadConcurrency, async (item, i) => {
  if (!item || !item.name || !item.blob) {
    sourceUploadFailures.push(`upload ${i + 1}: invalid upload entry`);
    await updateHitProgress(hit, Math.min(fileEntries.length + 1, i + 1), fileEntries.length + 1, `Uploaded ${i + 1} / ${sourceUploads.length}`);
    return;
  }
  try {
    const itemSize = binarySourceSize(item.blob);
    if (itemSize <= 0) {
      throw new Error('empty source file');
    }
    const delayMs = getPerItemDelayMs();
    if (delayMs) await sleep(delayMs);
    const ok = await uploadBlob(stagingFolder, item.name, item.blob, { signal: archiveSignal || undefined });
    if (!ok) throw new Error('upload rejected');
  } catch (err) {
    sourceUploadFailures.push(`${item.name}: ${err?.message || err}`);
  }
  await updateHitProgress(hit, Math.min(fileEntries.length + 1, i + 1), fileEntries.length + 1, `Uploaded ${i + 1} / ${sourceUploads.length}`);
});

  if (sourceUploadFailures.length) {
    const issueLines = Array.isArray(warnings) ? warnings : [];
    issueLines.push(...sourceUploadFailures.map((line) => `source upload failed: ${line}`));
    issueLines.push('server ffmpeg mode: fast-start MP4 remux');
    hit.error = issueLines.join(' | ');
    hit.status = sourceUploads.length > sourceUploadFailures.length ? 'partial' : 'failed';
    hit.archivedFiles = [];
    await saveHits().catch(() => {});
    await updateBadge().catch(() => {});
  ...
  try {
    hit.progressLabel = `Waiting for server ffmpeg remux slot • ${stagingFolder}`;
    void scheduleHitPersistence(false);
    scheduleBadgeUpdate(150);
    await yieldToEventLoop();
    compiled = await remuxPlaylistOnServer({
    ...
    const remuxError = String(err?.message || err || 'server ffmpeg remux failed');
    const issueLines = Array.isArray(warnings) ? warnings : [];
    issueLines.push(`server ffmpeg remux failed: ${remuxError}`);
    issueLines.push(`server ffmpeg remux inputs: ${playlistName}`);
    issueLines.push(`server ffmpeg staging folder: ${stagingFolder}`);
    issueLines.push(`server ffmpeg mode: fast-start MP4 remux`);

    const issuesBlob = new Blob([issueLines.join('\n') + '\n'], { type: 'text/plain' });
    await uploadBlob(folder, 'issues.txt', issuesBlob, { signal: archiveSignal || undefined }).catch(() => {});
    hit.error = issueLines.join(' | ');
    hit.status = 'failed';
    await saveHits().catch(() => {});
  ...
  mainOutputUploaded = Boolean(compiled?.uploaded);
  if (!mainOutputUploaded) {
    const failure = String(hit.error || `Archive upload failed for ${finalBlobName}.`);
    hit.status = 'failed';
    hit.error = failure;
    hit.archivedFiles = [];
    await saveHits().catch(() => {});
  ...
  await uploadBlob(folder, 'skip-meta.json', new Blob([JSON.stringify(meta, null, 2)], { type: 'application/json' }), { signal: archiveSignal || undefined }).catch(() => {});
...
async function remuxPlaylistOnServer({
  ...
  const { serverOrigin } = serverInternalBases();
  if (!serverOrigin) {
    throw new Error('Server not configured.');
  }

  const sourceBaseFolder = sourceFolder || folder;
  const targetBaseFolder = targetFolder || folder || sourceBaseFolder;
  const sourcePath = serverFileUrl(sourceBaseFolder, playlistName);
  const targetPath = serverFileUrl(targetBaseFolder, outputName);
  if (!sourcePath) throw new Error('Server source path unavailable.');
  if (!targetPath) throw new Error('Server target path unavailable.');

  const setStatus = (label) => {
    if (typeof onStatus === 'function') onStatus(label);
  };

  setStatus(`Verifying staged source • ${playlistName}`);
  await waitForServerFile(sourceBaseFolder, playlistName, {
    signal: archiveSignal || undefined,
    retries: 5,
    delayMs: 250
  });

  setStatus('Submitting server ffmpeg remux');
  const submit = await fetch(joinUrl(serverOrigin, '/api/ffmpeg/repair'), {
    method: 'POST',
    mode: 'cors',
    credentials: 'omit',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ sourcePath, sourceFolder: sourceBaseFolder, targetPath, outputName }),
    signal: archiveSignal || undefined
  });

  const submitData = await submit.json().catch(() => ({}));
  if (!submit.ok || submitData.ok === false) {
    throw new Error(submitData.error || `Server ffmpeg remux failed: HTTP ${submit.status}`);
  }

  const jobId = String(submitData?.job?.id || '').trim();
  if (!jobId) {
    throw new Error('Server ffmpeg remux did not return a job id.');
  }

  setStatus(`Server ffmpeg queued • ${jobId.slice(-6)}`);
  let lastLogCount = 0;
  while (true) {
    await sleep(1000);
    const poll = await fetch(joinUrl(serverOrigin, `/api/ffmpeg/repair/${encodeURIComponent(jobId)}`), {
      cache: 'no-store',
      credentials: 'omit',
      signal: archiveSignal || undefined
    });
    const data = await poll.json().catch(() => ({}));
    if (!poll.ok || data.ok === false) {
      throw new Error(data.error || `Server ffmpeg poll failed: HTTP ${poll.status}`);
    }

    const job = data.job || {};
    if (Array.isArray(job.logs) && job.logs.length > lastLogCount) {
      const newLines = job.logs.slice(lastLogCount).filter(Boolean);
      if (newLines.length && typeof onStatus === 'function') {
        onStatus(newLines[newLines.length - 1]);
      }
      lastLogCount = job.logs.length;
    }

    if (job.status === 'queued' || job.status === 'running') {
      setStatus(`Server ffmpeg remuxing • ${job.status}`);
      continue;
    }

    if (job.status === 'done') {
      setStatus('Server ffmpeg remux complete');
      return { uploaded: true, name: outputName, jobId, sourcePath, targetPath, job };
    }

    throw new Error(job.error || 'Server ffmpeg remux failed');
  }
}
...
async function archivePlaylistHit(hit, options = {}) {
  ...
  const stagingFolder = options.stagingFolder || buildArchiveJobFolder(hit);
  hit.archivedFolder = folder;
  if (options.targetFileUrl) hit.targetFileUrl = String(options.targetFileUrl || '').trim() || hit.targetFileUrl || '';
  if (options.targetFileName) hit.targetFileName = String(options.targetFileName || '').trim() || hit.targetFileName || '';
  if (options.targetFileStem) hit.targetFileStem = String(options.targetFileStem || '').trim() || hit.targetFileStem || '';
  ...
  hit.outputName = String(options.outputName || requestedBaseName || hit.outputName || '').trim();
  hit.fileName = finalArchiveName;
  ...
  if (!currentText) {
    try {
      const delayMs = getPerItemDelayMs();
      if (delayMs) await sleep(delayMs);
      currentText = await fetchText(currentUrl, { referrer: hit.pageUrl || currentUrl, headers: requestHeadersFor(currentUrl, { pageUrl: hit.pageUrl || currentUrl }), archiveSignal });
    } catch (err) {
  ...
  if (parsed.type === 'media' && MEMORY.settings.saveSegments) {
    ...
    const downloadOne = async (record, attemptNo = 1) => {
      ...
      const fetched = await fetchBlobMetaWithProgress(record.uri, {
        ...
        headers: requestHeadersFor(record.uri, { pageUrl: hit.pageUrl || currentUrl }),
        archiveSignal
      }, (p) => {
  ...
  const remuxRequested = MEMORY.settings.saveSegments && parsed.type === 'media' && allSegments.length > 0 && allSegmentsSaved;
  if (remuxRequested) setArchiveStage(hit, 'remuxing', 'Server ffmpeg remuxing');

  const manifestToUpload = omittedPlaylist || exactPlaylist || currentText;
  if (uploads.length) {
    const sourceUpload = uploads.find((item) => item && item.name === sourceName && item.targetDir === stagingFolder) || null;
    if (sourceUpload) {
      sourceUpload.blob = new Blob([manifestToUpload || currentText || ''], { type: 'application/vnd.apple.mpegurl' });
    }
  }
  if (manifestToUpload) {
    pushUploadIfEnabled(uploads, manifestName, new Blob([manifestToUpload], { type: 'application/vnd.apple.mpegurl' }));
  }
  if (remuxRequested) {
    for (const seg of segmentMeta) {
      if (!seg || !seg.localName || !seg.blob) continue;
      uploads.push({ name: seg.localName, blob: seg.blob, targetDir: stagingFolder });
    }
    const stagedExtraUploads = [...new Map([
      ...[...keyBlobs.entries()],
      ...[...mapBlobs.entries()]
    ])].map(([uri, blob]) => {
      const localName = localMap.get(uri);
      return localName && blob ? { name: localName, blob, targetDir: stagingFolder } : null;
    }).filter(Boolean);
    uploads.push(...stagedExtraUploads);
  }

  const desc = describePlaylist(masterText || currentText, currentUrl);
  const browserCombineError = remuxRequested ? '' : (warnings.find((w) => String(w || '').startsWith('server ffmpeg remux failed: ')) || '').replace(/^server ffmpeg remux failed:\s*/, '');
  const meta = {
    sourceUrl: hit.url,
    ...
    folder,
    remuxRequested,
    browserCombineRequested: !!remuxRequested,
    browserCombineSucceeded: !!archiveState.mainOutputUploaded && !!remuxRequested,
    browserCombineError,
    remuxInput: remuxRequested ? manifestName : '',
    remuxOutput: remuxRequested ? finalArchiveName : '',
    warnings
  };
  pushUploadIfEnabled(uploads, metaName, new Blob([JSON.stringify(meta, null, 2)], { type: 'application/json' }));

  if (warnings.length) {
    pushUploadIfEnabled(uploads, 'issues.txt', new Blob([warnings.join('\n') + '\n'], { type: 'text/plain' }));
  }
  ...
  const processRecord = async (record) => {
    ...
    if (cachedBlob) {
      try {
        ...
        await uploadBlob(folder, record.localName, record.blob, { signal: archiveSignal || undefined });
...
async function archiveDirectMediaHit(hit, options = {}) {
  ...
  try {
    const delayMs = getPerItemDelayMs();
    if (delayMs) await sleep(delayMs);
    const baseHeaders = requestHeadersFor(hit.url, { pageUrl: hit.pageUrl || hit.sourceUrl || hit.url });
    const attempts = [
      baseHeaders,
  ...
  pushUploadIfEnabled(uploads, 'meta.json', new Blob([JSON.stringify(meta, null, 2)], { type: 'application/json' }));
  if (warnings.length) {
    pushUploadIfEnabled(uploads, 'issues.txt', new Blob([warnings.join('\n') + '\n'], { type: 'text/plain' }));
  }
...
function musicExtractPortFromUrl(input) {
...
function musicSafeUrl(input, fallback) {
  try {
...
function musicNormalizeStorageUrl(input) {
  const raw = String(input || '').trim();
  if (!raw) return '';
  try {
    const url = new URL(raw.includes('://') ? raw : `http://${raw}`);
    if (!url.pathname || url.pathname === '/') {
      url.pathname = '/musiclib.html';
    }
    return url.href;
  } catch {
    return raw;
...
function musicBuildStorageUrl(baseOrigin) {
  const base = String(baseOrigin || '').trim();
  if (!base) return '';
  try {
    return new URL('musiclib.html', base.endsWith('/') ? base : `${base}/`).href;
  } catch {
    return '';
  }
}

function musicBuildProxyUrl(baseOrigin) {
  const base = String(baseOrigin || '').trim();
  if (!base) return '';
  try {
    return new URL('Musicproxy/proxy', base.endsWith('/') ? base : `${base}/`).href;
  } catch {
    return '';
  }
}

function musicServerRoute(pathname) {
  const base = String(MEMORY.config?.serverOrigin || '').trim();
  if (!base) return '';
  try {
    return new URL(String(pathname || '').replace(/^\/+/, ''), base.endsWith('/') ? base : `${base}/`).href;
  } catch {
    return '';
  }
}

function musicNormalizeSettings(raw = {}) {
  const serverOrigin = String(MEMORY.config?.serverOrigin || '').trim();
  const storageUrl = musicNormalizeStorageUrl(raw.musicStorageUrl || raw.storageUrl || '') || musicBuildStorageUrl(serverOrigin);
  const storageParsed = storageUrl ? musicSafeUrl(storageUrl, 'http://localhost/musiclib.html') : null;
  const proxyUrl = String(raw.musicProxyUrl || raw.proxyUrl || '').trim() || musicBuildProxyUrl(serverOrigin);
  const proxyPort = String(raw.musicProxyPort || raw.proxyPort || '').trim() || musicExtractPortFromUrl(proxyUrl);
  const next = {
    musicStorageUrl: storageUrl,
    musicProxyUrl: proxyUrl,
    musicProxyPort: proxyPort,
    musicLibHost: storageParsed ? `${storageParsed.protocol}//${storageParsed.hostname}` : '',
    musicLibPort: storageParsed ? (storageParsed.port || (storageParsed.protocol === 'https:' ? '443' : '80')) : '',
    musicLibPath: storageParsed ? `${storageParsed.pathname || '/musiclib.html'}${storageParsed.search || ''}${storageParsed.hash || ''}` || '/musiclib.html' : '/musiclib.html'
  };
  return next;
}

function musicResponseHeaders(sourceResponse) {
  const headers = {};
  if (sourceResponse && typeof sourceResponse === 'object') {
    for (const [key, value] of Object.entries(sourceResponse)) {
      headers[key] = value;
    }
  }
  return headers;
...
function musicResolveTransfer(request = {}) {
  const transferId = String(request?.transferId || '').trim();
  if (transferId && MUSIC_FLOW_STORE.has(transferId)) return MUSIC_FLOW_STORE.get(transferId);

  const pageUrl = String(request?.pageUrl || request?.url || '').trim();
  if (pageUrl) {
    try {
      const parsed = new URL(pageUrl);
      const fromQuery = parsed.searchParams.get('transferId');
      if (fromQuery && MUSIC_FLOW_STORE.has(fromQuery)) {
        return MUSIC_FLOW_STORE.get(fromQuery);
      }
    } catch {}
  }

  for (const item of MUSIC_FLOW_STORE.values()) {
    if (!item.delivered) return item;
  }
  return null;
}

async function musicProxyFetch(payload = {}) {
  const routeUrl = musicServerRoute('Musicproxy/proxy');
  if (routeUrl) {
    try {
  ...
  for (const [key, value] of Object.entries(headers)) {
    ...
    init.headers[key] = String(value);
  }

  if (payload.bodyText != null) {
    init.body = String(payload.bodyText);
  } else if (payload.bodyBase64 != null) {
    const binary = atob(String(payload.bodyBase64 || '').trim());
  ...
  if (String(payload.responseType || 'text') === 'arrayBuffer' || String(payload.responseType || '').toLowerCase() === 'binary') {
...
async function musicProxyDownload(payload = {}) {
  const routeUrl = musicServerRoute('Musicproxy/download');
  if (routeUrl) {
    try {
      const response = await fetch(routeUrl, {
        method: 'POST',
      ...
      const type = String(response.headers.get('content-type') || 'application/octet-stream').trim() || 'application/octet-stream';
  ...
  const result = await musicProxyFetch({
    ...payload,
    responseType: 'arrayBuffer'
  });
  if (!result.ok) return result;

  const type = String(result.headers?.['content-type'] || 'application/octet-stream').trim() || 'application/octet-stream';
  const bytes = (() => {
    const binary = atob(String(result.body || ''));
...
async function musicSetActivity(activity = {}) {

  const state = String(activity.state || activity.status || 'idle').toLowerCase();
  const title = String(activity.title || activity.phase || 'LibraryJS Music Archiver').trim();
  ...
  return { ok: true, state, title, detail };
}

async function musicStoreSettings(nextSettings = {}) {
  const normalized = musicNormalizeSettings(nextSettings);
  await chrome.storage.local.set({ [MUSIC_SETTINGS_KEY]: normalized });
  return normalized;
}

async function musicLoadStoredSettings() {
  const stored = await chrome.storage.local.get(MUSIC_SETTINGS_KEY);
  return stored?.[MUSIC_SETTINGS_KEY] || {};
}

async function musicFlowReady(payload = {}) {
  const record = musicResolveTransfer(payload);
  if (!record) {
  ...
  if (record.tabId != null) {
    try { await chrome.tabs.update(record.tabId, { active: true }); } catch {}
  }
  return {
    ok: true,
...
async function musicOpenFlowTarget(payload = {}, sender = null) {
  const transferId = String(payload.transferId || crypto.randomUUID()).trim();
  const targetUrl = String(payload.targetUrl || '').trim();
  if (!targetUrl) {
    return { ok: false, error: 'Missing targetUrl' };
  }
  ...
  MUSIC_FLOW_STORE.set(transferId, record);
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  (async () => {
    ...
    if (msg?.type === 'UD_PROXY_REQUEST') {
      try {
        const result = await musicProxyFetch(msg.payload || {});
        sendResponse(result);
      } catch (error) {
    ...
    if (msg?.type === 'UD_PROXY_DOWNLOAD') {
      try {
        const result = await musicProxyDownload(msg.payload || {});
        sendResponse(result);
      } catch (error) {
        sendResponse({ ok: false, error: error?.message || String(error) });
      }
    ...
    if (msg?.type === 'UD_FLOW_TRANSFER') {
      try {
        const result = await musicOpenFlowTarget(msg.payload || {}, sender);
        sendResponse(result);
      } catch (error) {
        sendResponse({ ok: false, error: error?.message || String(error) });
      }
    ...
    if (msg?.type === 'UD_FLOW_READY') {
      try {
        const result = await musicFlowReady(msg.payload || {});
        sendResponse(result);
      } catch (error) {
        sendResponse({ ok: false, error: error?.message || String(error) });
      }
    ...
    if (msg?.type === 'UD_ACTIVITY_STATE') {
      try {
        const result = await musicSetActivity(msg.payload || {});
        sendResponse(result);
      } catch (error) {
        sendResponse({ ok: false, error: error?.message || String(error) });
      }
    ...
    if (msg?.type === 'UD_SETTINGS_UPDATE') {
      try {
        const result = await musicStoreSettings(msg.payload || {});
    ...
    if (msg?.type === 'ARCHIVE_HIT') {
      ...
      const exactTargetFileName = String(
        msg.targetFileName ||
        fallback?.targetFileName ||
        (exactTargetFileUrl ? exactTargetFileUrl.split(/[\\/]/).pop() : '') ||
        hit.targetFileName ||
        ''
      ).trim();
      const exactTargetFileStem = String(
        msg.targetFileStem ||
        fallback?.targetFileStem ||
        (exactTargetFileName ? exactTargetFileName.replace(/\.[^.]+$/, '') : '') ||
        hit.targetFileStem ||
        ''
      ).trim();
      hit.targetFileUrl = exactTargetFileUrl;
      hit.targetFileName = exactTargetFileName;
      hit.targetFileStem = exactTargetFileStem;
      hit.outputName = String(msg.outputName || fallback?.outputName || hit.outputName || exactTargetFileStem || '').trim();
      hit.fileName = exactTargetFileName ? `${exactTargetFileName}` : (hit.outputName ? `${hit.outputName}` : String(fallback?.fileName || hit.fileName || '').trim());
      hit.status = 'queued';
      hit.error = '';
...
chrome.webRequest.onBeforeSendHeaders.addListener((details) => {
  ...
  MEMORY.requestHeaders.set(url, headers);
  try {
    const origin = new URL(url).origin;
    MEMORY.requestHeaders.set(`__origin__:${origin}`, headers);
  } catch {}
  const docUrl = String(details.documentUrl || details.initiator || '').trim();
  if (docUrl) {
    MEMORY.requestHeaders.set(docUrl, headers);
    try {
      const docOrigin = new URL(docUrl).origin;
      MEMORY.requestHeaders.set(`__origin__:${docOrigin}`, headers);
    } catch {}
  }
