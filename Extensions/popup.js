...
function hasMissingSegmentActions(hit) {
  ...
  const missingCount = Number(hit?.missingSegmentCount || 0);
  const missingUrls = Array.isArray(hit?.missingSegmentUrls) ? hit.missingSegmentUrls.filter(Boolean) : [];
  if (missingCount > 0 || missingUrls.length > 0) return true;
  const errorText = [hit?.error, hit?.progressLabel, hit?.segmentSummary]
    .map((value) => String(value || '').toLowerCase())
    .join(' | ');
  return /failed to fetch|xmlhttprequest is not defined|missing segment|segment .* failed|slice\s+\d+\/\d+\s+failed/i.test(errorText);
}
...
async function openMusicArchiver() {
  const state = await loadState();
  const configBase = normalizeBaseUrl(state?.config?.serverOrigin || state?.settings?.configBaseUrl || '');
  if (!configBase) {
    console.warn('No config base URL is set for Musicproxy.');
    return;
  }

  const activeUrl = await getActiveTabUrl();
  if (!isYouTubeLikeUrl(activeUrl)) {
    return;
  }

  const target = new URL('Musicproxy/index.html', configBase);
  const musicStorageUrl = new URL('musiclib.html', configBase).href;
  const musicProxyUrl = new URL('Musicproxy/proxy', configBase).href;
  const musicProxyPort = target.port || (target.protocol === 'https:' ? '443' : '80');

  target.searchParams.set('sourceUrl', activeUrl);
  target.searchParams.set('autostart', '1');
  target.searchParams.set('musicStorageUrl', musicStorageUrl);
  target.searchParams.set('musicProxyUrl', musicProxyUrl);
  target.searchParams.set('musicProxyPort', musicProxyPort);
...
$('musicBtn')?.addEventListener('click', async () => {
  try {
    await openMusicArchiver();
  } catch (err) {
    console.warn('Music Archiver launch failed:', err);
  }
});
