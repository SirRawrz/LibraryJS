...
const els = {
  ...
  btnOpenMusicLib: document.getElementById("btnOpenMusicLib"),
  musicLibHost: document.getElementById("musicLibHost"),
  musicLibPort: document.getElementById("musicLibPort"),
  musicLibPath: document.getElementById("musicLibPath"),
  musicProxyUrl: document.getElementById("musicProxyUrl"),
  musicProxyPort: document.getElementById("musicProxyPort"),
  musicLibStatus: document.getElementById("musicLibStatus"),
...
const SETTINGS_KEY = "ud_musiclib_handoff_settings";
let bridgeSettings = {
  musicProxyUrl: "",
  musicProxyPort: ""
};
...
function setMusicLibStatus(text, kind = "") {
  if (!els.musicLibStatus) return;
  els.musicLibStatus.value = text;
  els.musicLibStatus.className = `status ${kind}`.trim();
}
...
async function setBridgeActivity(state, payload = {}) {
  if (!window.MusicArchiverProxy?.setActivity) return;
  try {
    await window.MusicArchiverProxy.setActivity({
      state,
      ...payload
    });
  } catch {
...
function parseMusicStorageUrl(input) {
  ...
  try {
    ...
    return {
      ...
      path: `${url.pathname || "/musiclib.html"}${url.search || ""}${url.hash || ""}` || "/musiclib.html",
      url: url.href
    };
  } catch {
    return null;
...
function normalizeLoadedSettings(raw = {}) {
  ...
  if (parsed) {
    ...
    next.musicLibHost = parsed.host;
    next.musicLibPort = parsed.port;
    next.musicLibPath = parsed.path;
  } else {
    next.host = String(next.host || next.musicLibHost || "").trim();
    next.port = String(next.port || next.musicLibPort || "").trim();
    next.path = String(next.path || next.musicLibPath || "/musiclib.html").trim() || "/musiclib.html";
    next.musicLibHost = next.host;
    next.musicLibPort = next.port;
    next.musicLibPath = next.path;
  ...
  if (!next.musicProxyUrl && next.musicStorageUrl) {
    try {
      next.musicProxyUrl = new URL('Musicproxy/proxy', new URL(next.musicStorageUrl).origin.endsWith('/') ? new URL(next.musicStorageUrl).origin : `${new URL(next.musicStorageUrl).origin}/`).href;
    } catch {}
  }
...
function extractSettingsFromQuery() {
  try {
    ...
    const raw = {};
    const storageUrl = params.get("musicStorageUrl") || params.get("storageUrl") || params.get("musicLibUrl");
    ...
    const legacyHost = params.get("musicLibHost");
    const legacyPort = params.get("musicLibPort");
    const legacyPath = params.get("musicLibPath");
    ...
    if (legacyHost) raw.musicLibHost = legacyHost;
    if (legacyPort) raw.musicLibPort = legacyPort;
    if (legacyPath) raw.musicLibPath = legacyPath;
    return raw;
  } catch {
    return {};
  }
...
async function syncSettingsToBridge(next) {
  if (!window.MusicArchiverProxy?.updateSettings) return;
  try {
    await window.MusicArchiverProxy.updateSettings({
      musicStorageUrl: next.musicStorageUrl || buildMusicStorageUrl(next.musicLibHost, next.musicLibPort, next.musicLibPath),
      ...
      musicLibHost: next.musicLibHost,
      musicLibPort: next.musicLibPort,
      musicLibPath: next.musicLibPath
    });
  } catch {
...
function saveHandoffSettings() {
  const next = normalizeLoadedSettings({
    musicLibHost: String(els.musicLibHost?.value || "").trim(),
    musicLibPort: String(els.musicLibPort?.value || "").trim(),
    musicLibPath: String(els.musicLibPath?.value || "").trim(),
...
function normalizeMusicLibTarget(settings = currentHandoffSettings || {}, transferId = '') {
  const saved = settings || {};
  const storageUrl = String(saved.musicStorageUrl || "").trim();

  let host = String(els.musicLibHost?.value || saved.musicLibHost || saved.host || "").trim();
  let port = String(els.musicLibPort?.value || saved.musicLibPort || saved.port || "").trim();
  let path = String(els.musicLibPath?.value || saved.musicLibPath || saved.path || "").trim();
  ...
  if (!host && storageUrl) {
    try {
      ...
      if (!path) path = `${parsed.pathname || "/musiclib.html"}${parsed.search || ""}${parsed.hash || ""}` || "/musiclib.html";
    } catch {}
  }

  host = host || saved.musicLibHost || saved.host || "";
  port = port || saved.musicLibPort || saved.port || "";
  path = path || saved.musicLibPath || saved.path || "/musiclib.html";
  ...
  if (transferId) url.searchParams.set("transferId", transferId);
  if (saved.musicStorageUrl) url.searchParams.set("musicStorageUrl", saved.musicStorageUrl);
  if (saved.musicLibPort || port) url.searchParams.set("musicLibPort", saved.musicLibPort || port);
...
async function fetchStreamPayload(stream) {
  ...
  const response = await window.MusicArchiverProxy.request(window.MusicArchiverProxy.mediaGetRequest(stream.url));
...
async function sendStreamToMusicLib(stream) {
  ...
  const targetUrl = normalizeMusicLibTarget(currentHandoffSettings, transferId);
  const fileName = els.fileName.value.trim() || stream.filename || `${safeFileStem(title || "download")}${mediaExtFromMime(stream.mimeType || "")}`;

  console.log(`[MusicProxy] transferId: ${transferId}, targetUrl: ${targetUrl}`);

  setAutoProgress(82, "Opening MusicLib", "Launching the MusicLib flow page and preparing the file drop.", "Handoff");
  await setBridgeActivity("busy", { phase: "Opening MusicLib", progress: 82, title: "Downloading…" });

  const transfer = await window.MusicArchiverProxy.sendFlowTransfer({
    transferId,
    targetUrl,
    openTarget: true,
    fileName,
  ...
  setMusicLibStatus(`Opened ${transfer.targetUrl} and queued ${fileName} for the flow page.`, "ok");
  log(`flow transfer queued: ${fileName} -> ${transfer.targetUrl}`);
  return transfer;
}

function openMusicLibTarget() {
  currentHandoffSettings = loadHandoffSettings();
  const targetUrl = normalizeMusicLibTarget(currentHandoffSettings);
  if (!targetUrl) {
    setMusicLibStatus("No MusicLib storage URL is configured yet.", "err");
    return;
  }
  window.open(targetUrl, "_blank", "noopener");
  setMusicLibStatus(`Opened ${targetUrl}.`, "ok");
  log(`musiclib opened: ${targetUrl}`);
}
...
async function fetchPlayerAndDownloadSelected() {
  ...
  const request = window.MusicArchiverProxy.youtubePlayerRequest(videoId);
  ...
  await sendStreamToMusicLib(selected);

  setAutoProgress(100, "Handoff queued", "The MusicLib page is now receiving the file.", "Step 4 of 4");
...
async function sendRequest(request) {
  if (!window.MusicArchiverProxy) {
    throw new Error("Extension bridge not found");
  }
  previewRequest(request);
  setStatus("Sending request…");
  const response = await window.MusicArchiverProxy.request(request);
  return response;
}
...
async function downloadStream(stream) {
  ...
  const response = await window.MusicArchiverProxy.downloadMedia(stream.url, filename);
...
els.btnNext.addEventListener("click", async () => {
  try {
    const videoId = syncVideoField();
    const request = window.MusicArchiverProxy.youtubeNextRequest(videoId);
    const response = await sendRequest(request);
    const parsed = parseMaybeJson(response);
    els.jsonOutput.value = pretty(normalizeResponseBody(parsed));
...
if (els.btnSendSelected) {
  els.btnSendSelected.addEventListener("click", async () => {
    try {
      ...
      if (!stream) throw new Error("No stream selected");
      syncFieldsFromStream(stream);
      setAutoProgress(65, "Preparing handoff", "Fetching the selected stream and opening MusicLib.", "Manual handoff");
      await setBridgeActivity("busy", { phase: "Preparing MusicLib handoff", progress: 65, title: "Downloading…" });
      await sendStreamToMusicLib(stream);
      setStatus("Stream handed off to MusicLib.", "ok");
      scheduleAutoProgressHide(900);
    } catch (error) {
      setStatus(error.message || String(error), "err");
      setMusicLibStatus(error.message || String(error), "err");
      log(`musiclib transfer failed: ${error.message || error}`);
    }
  });
}

if (els.btnOpenMusicLib) {
  els.btnOpenMusicLib.addEventListener("click", () => {
    try {
      openMusicLibTarget();
      setStatus("MusicLib flow page opened.", "ok");
    } catch (error) {
      setStatus(error.message || String(error), "err");
      log(`musiclib open failed: ${error.message || error}`);
    }
  });
}
...
els.btnDownload.addEventListener("click", async () => {
  try {
    ...
    const request = window.MusicArchiverProxy.mediaGetRequest(url);
    previewRequest(request);
    setStatus("Starting download…");

    const response = await window.MusicArchiverProxy.downloadMedia(url, filename);
    log(`download OK: ${response.filename} (id ${response.downloadId}, mode ${response.mode || "unknown"})`);
    setStatus(`download completed (${response.mode || "unknown"})`, "ok");
  } catch (error) {
    setStatus(error.message || String(error), "err");
...
window.addEventListener("load", async () => {
  ...
  if (saved.musicLibHost && els.musicLibHost) els.musicLibHost.value = saved.musicLibHost;
  if (saved.musicLibPort && els.musicLibPort) els.musicLibPort.value = saved.musicLibPort;
  if (saved.musicLibPath && els.musicLibPath) els.musicLibPath.value = saved.musicLibPath;
  bridgeSettings = {
  ...
  [els.musicLibHost, els.musicLibPort, els.musicLibPath, els.musicProxyUrl, els.musicProxyPort].forEach((el) => {
    if (!el) return;
    el.addEventListener("change", saveHandoffSettings);
    el.addEventListener("input", () => {
      saveHandoffSettings();
      if (els.musicLibStatus) {
        els.musicLibStatus.value = `Target: ${normalizeMusicLibTarget(currentHandoffSettings)}`;
      }
    });
  });

  if (els.musicLibStatus) {
    els.musicLibStatus.value = `Target: ${normalizeMusicLibTarget(saved)}`;
  }
  ...
  if (window.MusicArchiverProxy) {
    ...
    previewRequest(window.MusicArchiverProxy.youtubeNextRequest(syncVideoField()));
    ...
    if (shouldAutoStartTask()) {
      ...
      try {
        ...
        await setBridgeActivity("idle", { phase: "Idle", title: "Music Archiver Relay" });
