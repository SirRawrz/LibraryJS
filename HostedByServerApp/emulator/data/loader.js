(async function() {
	...
	config.secondUrl = window.EJS_secondUrl;
    ...
    if (window.EJS_dualConfig && window.EJS_dualConfig.romParam) {
        ...
        // --- FIX IS HERE: Changed EJS_player to window.EJS_player ---
// --- THIS IS THE NEW, CORRECT CODE ---
var playerElement;
if (window.EJS_module && window.EJS_module.canvas && window.EJS_module.canvas.id) {
    // Use the canvas's ID as a string selector
    playerElement = "#" + window.EJS_module.canvas.id;
} else {
    // Fall back to the default EJS_player selector (e.g., "#game")
    playerElement = window.EJS_player;
}
window.EJS_emulator = new EmulatorJS(playerElement, config);
