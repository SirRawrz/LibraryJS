...
internal sealed class LibraryJSServerContext : ApplicationContext
{
    ...
    private IReadOnlyList<ActiveEndpointGroup> BuildActiveEndpointGroups()
    {
        ...
        foreach (var instance in _servers.Where(server => !server.Process.HasExited))
        {
            ...
            var title = index == 0 ? "Main Server" : $"Expanded {index}";
...
public MainWindow(int defaultPort, Icon appIcon)
{
    ...
    var activeEndpointsTitle = new Label
    {
        Left = 18,
        Top = 14,
        Width = 360,
    ...
    var activeEndpointsHint = new Label
    {
        Left = 18,
        Top = 42,
        Width = 1060,
    ...
    _storageLinksButton = CreateActionButton("Show all storage links", _activeEndpointsCard.ClientSize.Width - 224, 12, 206, Color.FromArgb(46, 62, 63));
    _storageLinksButton.Click += (_, _) => OpenStorageLinksDialog();

    _activeEndpointsCard.Controls.Add(activeEndpointsTitle);
    _activeEndpointsCard.Controls.Add(activeEndpointsHint);
    _activeEndpointsCard.Controls.Add(_activeEndpointsPanel);
    _activeEndpointsCard.Controls.Add(_storageLinksButton);
    ...
    private void LayoutDashboard()
    {
        ...
        _storageLinksButton.SetBounds(Math.Max(18, _activeEndpointsCard.ClientSize.Width - 224), 12, 206, 32);

        var footerBottom = Math.Min(maxHeight - 16, ClientSize.Height - 16);
        _buttonPanel.SetBounds(margin, footerBottom - 42, width, 42);
        _startButton.Left = Math.Max(0, width - (_startButton.Width + _startupOptionsButton.Width + _exitButton.Width + 24));
        _startupOptionsButton.Left = _startButton.Right + 12;
        _exitButton.Left = _startupOptionsButton.Right + 12;

        _statusDetailLabel.SetBounds(margin, _buttonPanel.Top - 24, width, 18);
        _statusLabel.SetBounds(margin, _statusDetailLabel.Top - 22, width, 18);
