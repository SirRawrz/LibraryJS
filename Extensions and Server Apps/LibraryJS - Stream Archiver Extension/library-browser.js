(() => {
  ...
  function getFolderStats(node) {
    ...
    for (const ep of episodes) {
      const st = getFileAvailability(ep.file);
    ...
    return { total, checked, exists, missing, checking, done, label: done ? (missing > 0 ? `${missing} missing` : `${exists}/${total} found`) : `Check folder` };
  }
  ...
  function episodeActionClass(node) {
    const st = getFileAvailability(node.file);
  ...
  async function checkFolder(node) {
    ...
    const firstMissing = leaves.find(ep => getFileAvailability(ep.file).status !== 'exists');
    if (firstMissing) {
  ...
  function selectNode(pathKey) {
    ...
    if (node.file) {
      ...
      if (!existing) checkAvailabilityForNode(node);
    }
    updateFooter();
    renderTree();
  }
